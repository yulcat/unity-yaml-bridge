...
exports.writeCompact = writeCompact;
...
function expandNestedPrefab(node, ctx) {
    ...
    if (!sourceGuid || ctx.visited.has(sourceGuid))
        return null;
    ...
    try {
        ...
        const sourceFile = (0, unity_yaml_parser_1.parseUnityYaml)(sourceContent);
...
function resolveSourceName(node, resolver) {
    if (!node.nestedPrefab)
        return '';
...
function writeDetails(node, lines, path, resolver, filterBoilerplate = true, refMap, selectors) {
    ...
    for (const comp of node.components) {
        ...
        const propEntries = Object.entries(props).filter(([k, v]) => {
            ...
            if (filterBoilerplate && COMPACT_OMIT_FIELDS.has(k))
                return false;
            // Filter null material references
            if (filterBoilerplate && k === 'm_Material' && isNullRef(v))
                return false;
            return true;
        });
...
function removeAmbiguousRefPaths(map) {
...
function buildStrippedComponentMap(file, resolver, nestedObjectRefs = buildNestedSourceObjectRefMap(file.documents, file.hierarchy, resolver)) {
    const map = new Map();
 ...
 */
function buildNestedSourceObjectRefMap(docs, hierarchy, resolver) {
    ...
    for (const doc of docs) {
        ...
        if (!piRef || !sourceRef?.guid || sourceRef.fileID === undefined)
            continue;
        const piFileId = String(piRef.fileID);
...
function writeTransformSection(transform, path) {
...
function isDefaultRotation(rot) {
...
function numEq(a, b) {
    ...
    if (typeof a === 'number') {
        if (b === 0)
            return a === 0 || Object.is(a, -0);
        return a === b;
    }
    return false;
}
/** Resolve a component name from its info */
function resolveComponentName(comp, resolver) {
    if (comp.typeId !== 114)
...
function resolveBaseChain(guid, resolver, visited = new Set()) {
...
function collectNestedNodeNames(node, map, resolver) {
    if (node.nestedPrefab) {
        let name = node.name;
...
function inferComponentType(propertyPaths) {
 ...
 */
function resolveNestedTargets(unresolvedTargets, baseHierarchy, basePrefabInstances, resolver) {
    ...
    for (const [targetId, mods] of unresolvedTargets) {
        ...
        if (!compType)
            continue;
        if (!targetsByType.has(compType))
            targetsByType.set(compType, []);
        ...
        for (const [targetId, mods] of targets) {
            ...
            for (let i = 0; i < candidates.length; i++) {
                ...
                for (const p of variantPaths) {
                    if (candidates[i].baseModPropPaths.has(p))
                        overlap++;
                }
 ...
 */
function collectSourceEntries(parentPath, sourceGuid, piMods, resolver, result, depth) {
    if (depth > 3)
        return;
    ...
    try {
        ...
        for (const [, info] of sourceDocInfo) {
            if (skipTypes.has(info.componentType))
                continue;
        ...
        for (const mod of piMods) {
            ...
            if (!modsByTarget.has(id))
                modsByTarget.set(id, new Set());
            modsByTarget.get(id).add(mod.propertyPath);
        }
        // Build stripped doc → sub-PI mapping
        const strippedToPI = new Map();
        for (const doc of sourceFile.documents) {
            ...
            if (docInfo) {
                ...
                if (subPiId && !processedSubPIs.has(subPiId)) {
                    ...
                    if (subPi && subPi.sourcePrefab.guid) {
                        ...
                        collectSourceEntries(`${parentPath}/${subName}`, subPi.sourcePrefab.guid, subPi.modifications, resolver, result, depth + 1);
                    }
                }
                else if (!subPiId) {
        ...
        for (const subPi of sourceFile.prefabInstances) {
            if (processedSubPIs.has(subPi.fileId))
                continue;
            processedSubPIs.add(subPi.fileId);
            if (!subPi.sourcePrefab.guid)
                continue;
...
function countNestedComponentTypes(hierarchy, prefabInstances, resolver, typeCount) {
    const skipTypes = new Set(['GameObject', 'Transform', 'RectTransform', 'CanvasRenderer']);
...
function resolveTargetKey(targetFileId, baseMap) {
...
function resolveAddedComponents(file, mainInstance, mainSourceMap, resolver) {
    ...
    for (const instance of file.prefabInstances) {
        ...
        for (const added of instance.addedComponents) {
            ...
            if (!document || document.stripped || document.typeId === 1 || document.typeId === 1001)
                continue;
...
function buildAddedComponentOverlay(components, instances) {
    ...
    const byUniqueSource = new Map();
    const sourceInstanceCounts = new Map();
    for (const instance of instances) {
        const guid = instance.sourcePrefab.guid || '';
        if (!sourceInstanceCounts.has(guid))
            sourceInstanceCounts.set(guid, new Set());
...
function buildVariantRemovalOverlay(file, mainInstance, mainSourceMap, resolver) {
    ...
    for (const instance of file.prefabInstances) {
        ...
        for (const removed of instance.removedGameObjects) {
            ...
            if (goFileId)
                gameObjects.add(`${sourceGuid}:${goFileId}`);
        }
    }
    return { components, gameObjects };
}
...
function writeAddedComponentDetails(components, lines, refMap) {
    for (const component of components) {
        const entries = Object.entries(component.document.properties).filter(([key, value]) => {
            if (types_1.OMIT_FIELDS.has(key) || COMPACT_OMIT_FIELDS.has(key))
                return false;
            if (key === 'm_Material' && isNullRef(value))
                return false;
            return true;
        });
        if (entries.length === 0)
...
function writeAddedComponentRefs(components, lines) {
    for (const component of components) {
...
function writeVariantCompact(file, lines, resolver, version = 1) {
        ...
        if (resolvedBase) {
            try {
                ...
                if (unresolvedTargets.size > 0 && baseHierarchy) {
                    nestedResolved = resolveNestedTargets(unresolvedTargets, baseHierarchy, baseFile.prefabInstances, resolver);
                }
            }
            catch {
    ...
    for (const target of inheritedModifiedTargets)
    ...
    const addedComponentRefMap = file.hierarchy?.name === path_utils_1.ADDED_ROOT_NAME
...
function resolveVariantReferenceKey(targetId, baseMap, nestedResolved) {
    if (baseMap) {
...
function formatVariantModificationValue(mod, baseMap, nestedResolved) {
...
function resolveVariantHeader(targetId, mods, baseMap, nestedResolved, preferNameOverrideHeader = false) {
    ...
    if (componentType)
        return `&${targetId}:${componentType}`;
    return `&${targetId}`;
}
/** Write variant REFS section */
function writeVariantRefs(instance, lines, baseMap, nestedResolved, preferNameOverrideHeader = false, ownerInstanceId) {
    ...
    const emitRef = (key, targetId, includeOwner) => {
