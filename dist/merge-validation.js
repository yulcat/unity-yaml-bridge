...
exports.cloneCompactFile = cloneCompactFile;
exports.assertCompactSourceCompatible = assertCompactSourceCompatible;
exports.collectUnityIntegrityIssues = collectUnityIntegrityIssues;
exports.assertNoNewIntegrityIssues = assertNoNewIntegrityIssues;
...
function cloneCompactFile(compact) {
    return {
...
function assertCompactSourceCompatible(original, compact) {
    if (original.type !== compact.type) {
        throw new Error(`Compact/YAML type mismatch: compact is ${compact.type}, original YAML is ${original.type}.`);
    }
    if (compact.type === 'variant' && compact.baseGuid && compact.baseGuid !== 'unknown') {
        ...
        if (sourceGuid && sourceGuid !== compact.baseGuid) {
            throw new Error(`Compact/YAML source mismatch: base GUID ${compact.baseGuid} does not match ${sourceGuid}.`);
        }
    }
    if (compact.type === 'variant' || !compact.structure || !original.hierarchy)
        return;
    if (compact.structure.name !== original.hierarchy.name) {
        throw new Error(`Compact/YAML root mismatch: ${compact.structure.name} does not match ${original.hierarchy.name}.`);
    }
    ...
    if (rootRefs && original.hierarchy.fileId !== '0' && !rootRefs.includes(original.hierarchy.fileId)) {
        throw new Error(`Compact/YAML root identity mismatch: ${compact.structure.name} expects ` +
...
function collectUnityIntegrityIssues(file) {
    const issues = new Set();
    const ids = new Set();
    const docsById = new Map();
    for (const document of file.documents) {
        if (ids.has(document.fileId))
            issues.add(`duplicate document &${document.fileId}`);
        ...
        if (document.typeId === 1 && !document.stripped) {
            ...
            for (const entry of entries) {
                ...
                if (ownerId !== '0' && ownerId !== document.fileId) {
                    issues.add(`GameObject &${document.fileId} attaches &${componentId}, owned by &${ownerId}`);
                }
            }
        }
...
function assertNoNewIntegrityIssues(baseline, merged) {
