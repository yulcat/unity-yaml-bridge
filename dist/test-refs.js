...
if (fs.existsSync(projectPath)) {
    ...
    resolver.addAsset('2982fa53447c5c643865bbd0d194eab1', path.join(SAMPLES_DIR, 'prefabs', '_Card_Template.prefab'), '_Card_Template');
...
console.log('='.repeat(60));
{
    ...
    const compact = (0, compact_reader_1.readCompact)(compactStr);
    const hasResolvedHeaders = compact.sections.some(s => !s.goPath.startsWith('&') && s.goPath !== '');
...
console.log('='.repeat(60));
{
    const ids = new Set();
    for (let i = 0; i < 100; i++) {
        ids.add((0, compact_merger_1.generateFileId)());
    }
...
console.log('='.repeat(60));
{
    ...
    // YAML roundtrip
    const output = (0, unity_yaml_writer_1.writeUnityYaml)(ast);
    const origLines = content.split('\n').map(l => l.trimEnd());
    const outLines = output.split('\n').map(l => l.trimEnd());
    let diffs = 0;
    for (let i = 0; i < Math.max(origLines.length, outLines.length); i++) {
...
//# sourceMappingURL=test-refs.js.map