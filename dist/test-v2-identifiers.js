...
function fixture(childOrder = ['120', '110', '130'], componentOrder = ['900', '800']) {
...
assert(v2.includes('Inventory/Item#1:CanvasGroup = 210') &&
    v2.includes('Inventory/Item#2:CanvasGroup = 220'), 'REFS bind each sibling selector to one document');
assert(v2.includes('Inventory/Panel:CanvasGroup#1 = 800') &&
    v2.includes('Inventory/Panel:CanvasGroup#2 = 900'), 'REFS bind each component selector to one document');
assert(v2.includes('m_Target = ->Inventory/Item#1:CanvasGroup'), 'internal references use the same numbered selector');
const reordered = (0, compact_writer_1.writeCompact)((0, unity_yaml_parser_1.parseUnityYaml)(fixture(['110', '120', '130'], ['800', '900'])), { version: 2 });
...
assert(values.get('210') === 0.31 && values.get('220') === 0.32, 'write-back edits the intended duplicate sibling components');
assert(values.get('800') === 0.41 && values.get('900') === 0.42, 'write-back edits the intended same-type components');
const literal = (0, compact_writer_1.writeCompact)((0, unity_yaml_parser_1.parseUnityYaml)(fixture().replace(/m_Name: Item/g, 'm_Name: Item#1')), { version: 2 });
assert(literal.includes('Item#1#1') && literal.includes('Item#1#2'), 'a literal name ending in #N receives a separate final discriminator');
try {
    (0, compact_reader_1.readCompact)(v2.replace('Inventory/Item#1:CanvasGroup = 210\n', ''));
    assert(false, 'v2 numbered DETAILS require an exact REFS binding');
}
catch (error) {
    assert(String(error.message).includes('exactly one matching REFS'), 'v2 numbered DETAILS require an exact REFS binding');
}
try {
    const redirected = v2.replace('Inventory/Item#1:CanvasGroup = 210', 'Inventory/Item#1:CanvasGroup = 220');
    (0, compact_merger_1.mergeCompactChanges)(ast, (0, compact_reader_1.readCompact)(redirected));
    ...
    assert(String(error.message).includes('target GameObject not found') ||
        String(error.message).includes('ownership mismatch'), 'v2 REFS cannot redirect an alias to another sibling');
}
...
if (fs.existsSync(arenaPath)) {
    try {
        (0, compact_writer_1.writeCompact)((0, unity_yaml_parser_1.parseUnityYaml)(fs.readFileSync(arenaPath, 'utf8')), { version: 2 });
        assert(false, 'unresolved multi-owner nested selectors fail closed');
    }
    catch (error) {
        assert(String(error.message).includes('owner identity'), 'unresolved multi-owner nested selectors fail closed');
    }
}
console.log(`\nv2 identifier tests: ${passed} passed, ${failed} failed`);
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-v2-identifiers.js.map