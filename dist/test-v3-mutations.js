...
│  └─ Leaf @g4
└─ ParentB @g3`;
function v3(structure = baselineStructure, target = '{"$ref":"g4"}', extraIdentity = '') {
...
function compile(text) {
    return (0, unity_yaml_parser_1.parseUnityYaml)((0, unity_yaml_writer_1.writeUnityYaml)((0, compiler_1.compileV3)((0, reader_1.readV3)(text))));
}
function document(file, fileId) {
...
expectThrow(() => (0, reader_1.readV3)(v3().replace('g1 = gameObject | fileID:10', 'g1 = gameObject | origin:local | fileID:10')), 'Invalid v3 identity origin', 'unknown IDENTITY origin values are rejected instead of normalized');
{
    ...
    assert(String(document(file, '12')?.properties.target?.fileID) === '40', 'machine reference resolves to the target GameObject fileID');
}
{
    const renamed = compile(v3(baselineStructure.replace('Leaf @g4', 'RenamedLeaf @g4')));
    assert(document(renamed, '40')?.properties.m_Name === 'RenamedLeaf', 'rename changes display state while preserving GameObject fileID');
    assert(document(renamed, '41')?.properties.m_GameObject?.fileID === 40, 'rename preserves Transform fileID and ownership');
}
{
...
└─ ParentB @g3
   └─ Leaf @g4`;
    ...
    assert(String(document(moved, '41')?.properties.m_Father?.fileID) === '31', 'reparent rewrites m_Father while preserving child identity');
    assert((document(moved, '31')?.properties.m_Children || [])
        .some((entry) => String(entry.fileID) === '41'), 'reparent rewrites the desired parent m_Children list');
}
{
...
└─ ParentA @g2
   └─ Leaf @g4`;
    ...
    assert(rootChildren.map((entry) => String(entry.fileID)).join(',') === '31,21', 'STRUCTURE sibling order is authoritative');
}
{
...
└─ ParentB @g3`;
    ...
    assert(!document(deleted, '40') && !document(deleted, '41'), 'removing a subtree from STRUCTURE removes its Unity documents');
    assert((0, reader_1.readV3)(v3(deletedStructure, '{"fileID":0}')).identity.has('g4'), 'deleted entity identity remains available as baseline metadata');
    expectThrow(() => compile(v3(deletedStructure)), 'Dangling v3 reference', 'deleting a referenced entity fails unless the reference changes atomically');
}
{
    const withoutComponent = compile(v3(baselineStructure.replace(' [BoxCollider @c1]', ''), '{"fileID":0}'));
    assert(!document(withoutComponent, '12') &&
        document(withoutComponent, '10')?.properties.m_Component.length === 1, 'removing a component binding removes its document and attachment');
}
{
...
`;
    ...
    assert(!!addedFirst && addedFirst.fileId === addedSecond?.fileId, 'new GameObject receives a deterministic fileID without original YAML');
    const addedTransform = first.documents.find(item => (item.typeId === 4 || item.typeId === 224) &&
        String(item.properties.m_GameObject?.fileID) === addedFirst?.fileId);
    assert(!!addedTransform, 'new GameObject receives its structural Transform document');
}
{
    const withLight = baselineStructure.replace('[BoxCollider @c1]', '[BoxCollider @c1, Light @c2]');
    const light = compile(v3(withLight, '{"$ref":"g4"}', 'c2 = component | type:108 | typeName:Light | owner:g1\n'));
    const lightDocument = light.documents.find(item => item.typeId === 108);
    assert(!!lightDocument && String(lightDocument.properties.m_GameObject?.fileID) === '10', 'new native component is compiled and attached without original YAML');
}
console.log(`\nv3 mutation tests: ${passed} passed, ${failed} failed`);
if (failed > 0)
    process.exit(1);
//# sourceMappingURL=test-v3-mutations.js.map