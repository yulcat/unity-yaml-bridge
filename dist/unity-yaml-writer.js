...
function writeYamlProperties(props, lines, indent) {
    for (const [key, value] of Object.entries(props)) {
        writeYamlValue(key, value, lines, indent);
    }
}
/** Write a single key-value pair in YAML */
function writeYamlValue(key, value, lines, indent) {
...
/** Write an array item */
function writeYamlArrayItem(item, lines, indent) {
    const indentStr = ' '.repeat(indent);
...
function writeMultiLineFlowMapping(key, obj, lines, indent) {
    const indentStr = ' '.repeat(indent);
...
function writeMultiLineFlowMappingArrayItem(obj, lines, indent) {
    const indentStr = ' '.repeat(indent);
    const { first, rest } = splitFlowMapping(obj);
...
function formatFlowMapping(obj) {
    const parts = [];
...
function formatInlineValue(value) {
    ...
    if (typeof value === 'boolean')
        return value ? '1' : '0';
    if (typeof value === 'object' && isFlowMapping(value))
        return formatFlowMapping(value);
    if (Array.isArray(value))
...
function breakLongScalar(value, maxFirstLen, maxContLen) {
...
function findBreakPoint(str, maxLen, breakChar) {
...
//# sourceMappingURL=unity-yaml-writer.js.map