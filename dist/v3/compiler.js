...
function validateRawModificationEnvelope(entry, context) {
    ...
    const target = (0, references_1.validateV3ExternalObjectReference)(modification.target, `${context}.target`);
    ...
    const referenceFileId = validateRawModificationObjectReference(modification.objectReference, `${targetKey}.${propertyPath}`);
    if (keys.join(',') !== 'objectReference,propertyPath,target,value') {
        throw new Error(`Invalid raw modification envelope at ${context}.`);
    }
    if (modification.value !== '' && referenceFileId !== '0') {
        throw new Error(`Raw modification ${targetKey}.${propertyPath} mixes a nonempty scalar value with a nonzero objectReference.`);
    }
    return { targetGuid, targetKey, propertyPath };
}
function validateAddedObjectReference(value, context) {
    if (!isPlainRecord(value))
...
function validateVariantModificationContainers(document) {
    const fields = [
    ...
    for (const identity of document.identity.values()) {
        ...
        if (!details || !Object.prototype.hasOwnProperty.call(details, 'm_Modification'))
            continue;
        ...
        for (const field of fields) {
            if (!Object.prototype.hasOwnProperty.call(modification, field))
                continue;
            ...
            entries.forEach((entry, index) => {
                ...
                if (field === 'm_RemovedGameObjects' || field === 'm_RemovedComponents') {
                    (0, references_1.validateV3ExternalObjectReference)(entry, context);
                    return;
                }
                if (!isPlainRecord(entry) ||
                ...
                (0, references_1.validateV3ExternalObjectReference)(entry.targetCorrespondingSourceObject, `${context}.targetCorrespondingSourceObject`);
                validateAddedObjectReference(entry.addedObject, `${context}.addedObject`);
            });
        }
    }
}
function validateInheritedOwnerSourceUniqueness(document) {
    const identitiesByOwnerSource = new Map();
    for (const identity of document.identity.values()) {
        ...
        if (existing) {
            throw new Error(`Inherited identities ${existing.machineId} and ${identity.machineId} have a duplicate inherited ` +
...
function compileVariant(document) {
    ...
    const inheritedTransformStubs = new Map();
    ...
    const removedInheritedNestedGameObjects = new Set();
    const removedInheritedNestedComponents = new Set();
    ...
    const queueInheritedNestedGameObjectRemoval = (identity) => {
        ...
        if (removedInheritedNestedGameObjects.has(identity.machineId))
            return;
    ...
    const queueInheritedNestedComponentRemoval = (identity) => {
        ...
        if (removedInheritedNestedComponents.has(identity.machineId))
            return;
    ...
    const assertUniqueInheritedSourceTarget = (identity, operation) => {
        const matches = [...document.identity.values()].filter(candidate => candidate.kind === identity.kind && candidate.origin === 'inherited' &&
        ...
        if (matches.length !== 1 || matches[0].machineId !== identity.machineId) {
            throw new Error(`Inherited nested ${operation} target ${identity.sourceGuid}:${identity.sourceFileId} ` +
                'has an ambiguous owner/source path.');
        }
    };
    const requireLeafOwnedTransformStub = (transform) => {
        ...
        const fileId = allocateSyntheticFileId(document, `inherited-transform:${transform.machineId}`, allocated);
        documents.push({
    ...
    const emitLocalComponentOnInheritedGameObject = (gameObject, component) => {
        if (component.ownerId !== gameObject.machineId ||
            component.prefabOwnerId !== document.variantRootId) {
            throw new Error(`Local component ${component.machineId} on inherited GameObject ${gameObject.machineId} ` +
        ...
        if (!strippedGameObjectId) {
            ...
            if (existingStub) {
                ...
                strippedGameObjectId = allocateSyntheticFileId(document, `inherited-game-object:${gameObject.machineId}`, allocated);
                documents.push({
                    typeId: 1,
                    typeName: 'GameObject',
    ...
    const validateInheritedNestedInternal = (node, prefabOwnerId, nestedSourceGuid, parentTransformMachineId, siblingIndex, sourceRoot = false) => {
        ...
        const desiredInheritedComponents = node.components.filter(component => document.identity.get(component.machineId)?.origin === 'inherited');
        const desiredBaselineComponents = baselineComponents.filter(identity => desiredComponentIds.has(identity.machineId));
        ...
        node.children.forEach((child, index) => {
            ...
            if (childIdentity.origin === 'inherited' || child.nestedSourceGuid) {
                validateInheritedNestedInternal(child, prefabOwnerId, nestedSourceGuid, transform.machineId, inheritedChildIndex++);
                return;
            }
            if (childIdentity.kind !== 'gameObject' ||
                childIdentity.prefabOwnerId !== document.variantRootId) {
                throw new Error(`Local GameObject ${child.machineId} below inherited nested ${node.machineId} ` +
                    'requires the emitted leaf PrefabInstance owner.');
            }
            ...
            buildNode(child, requireLeafOwnedTransformStub(transform), index, transform.machineId);
        });
    };
    const buildNode = (node, parentTransformId, siblingIndex, parentTransformMachineId) => {
        if (node.tombstone) {
    ...
    const nestedGameObjects = [...document.identity.values()].filter(identity => identity.kind === 'gameObject' && identity.origin === 'inherited' && identity.prefabOwnerId &&
    ...
    const baselineParentGameObject = (identity) => {
        const transform = findOwnedTransform(document, identity.machineId);
        if (!transform.baselineParentId)
            return undefined;
    ...
    for (const identity of nestedGameObjects) {
        if (!missingNestedGameObjects.has(identity.machineId))
            continue;
        const parentGameObjectId = baselineParentGameObject(identity);
        if (parentGameObjectId && missingNestedGameObjects.has(parentGameObjectId))
            continue;
        queueInheritedNestedGameObjectRemoval(identity);
    }
    const removalSuppressesIdentity = (identity) => {
    ...
    for (const identity of document.identity.values()) {
        ...
        if (identity.machineId === document.variantRootId && hasInheritedStructure) {
            ...
            if (Array.isArray(modification.m_Modifications)) {
                ...
                modification.m_Modifications = modification.m_Modifications.filter((entry, index) => {
                    const { targetGuid, targetKey, propertyPath } = validateRawModificationEnvelope(entry, `m_Modification.m_Modifications[${index}]`);
                    ...
                    if (conflict) {
                        throw new Error(`Raw modification ${targetKey}.${propertyPath} overlaps another property path ${conflict}.`);
                    }
                    existingPaths.push(propertyPath);
                    rawSourcePaths.set(targetKey, existingPaths);
                    if (!inheritedSourceGuids.has(targetGuid) && (0, override_validation_1.isV3OverrideStructuralPath)(propertyPath)) {
                        throw new Error(`Raw unrelated modification ${targetKey}.${propertyPath} is structural and not supported.`);
                    }
