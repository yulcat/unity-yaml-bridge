...
import {
  assertCompactSourceCompatible,
  assertNoNewIntegrityIssues,
  cloneCompactFile,
  collectUnityIntegrityIssues,
...
function effectiveSectionComponentType(section: CompactSection): string | undefined {
  if (section.componentType) return section.componentType;
...
function validatePrefabOwnership(
  section: CompactSection,
  document: UnityDocument,
  goMap: Map<string, GameObjectNode[]>,
  file: UnityFile,
  ...
  if (!node) {
    ...
    const ownerId = getRefsValue(
      refs,
      `${sectionAddress(section)}:__instance`,
      { allowAddedRootAliases: true }
    ) || getRefsValue(
      refs,
      `${section.goPath}:__instance`,
      { allowAddedRootAliases: true }
    );
    const owner = ownerId
      ? file.prefabInstances.find(instance => instance.fileId === ownerId)
      : undefined;
    const attached = owner?.addedComponents.some(entry =>
      String(entry.addedComponent?.fileID) === document.fileId
    );
    if (section.isAdded && attached) return;
    throw new Error(`DETAILS target GameObject not found: ${section.goPath}.`);
  }
  if (document.stripped) return;

  let owned = false;
  if (!section.componentType) owned = document.fileId === node.fileId;
  ...
  if (!owned) {
    throw new Error(
      `REFS ownership mismatch for ${sectionAddress(section)}: ` +
...
export function mergeCompactChanges(
  ...
  const workingCompact = cloneCompactFile(compact);
  assertCompactSourceCompatible(original, workingCompact);
  const baselineIntegrity = collectUnityIntegrityIssues(original);
  ...
  assertNoNewIntegrityIssues(baselineIntegrity, result);
  ...
  for (const section of sections) {
    ...
    if (refsEntry && refsEntry.value.length > 0) {
      ...
      if (refsFileId) {
        ...
        throw new Error(
          `Stale REFS target for ${sectionAddress(section)}: document &${refsFileId} ` +
    ...
    if (candidates.length > 1) throw new Error(`Ambiguous GameObject path: ${goPath}.`);
    ...
    if (compType === 'Transform' || compType === 'RectTransform') {
      ...
      } else throw new Error(`Transform document not found for DETAILS target: ${goPath}.`);
    } else {
 ...
 */
function registerHierarchyRefAliases(
  node: GameObjectNode,
  refs: Map<string, string[]>,
  parentPath: string = ''
): void {
  const path = parentPath ? `${parentPath}/${node.name}` : node.name;
  if (!refs.has(path)) refs.set(path, [node.fileId]);

  const aliases = new Map<string, string[]>();
  const addAlias = (type: string | undefined, fileId: string): void => {
    if (!type) return;
    const key = `${path}:${type}`;
  ...
  for (const [key, ids] of aliases) {
    if (!refs.has(key)) refs.set(key, ids);
  }

  for (const child of node.children) registerHierarchyRefAliases(child, refs, path);
}
...
function mergeVariantSections(
  ...
  for (const section of sections) {
    ...
    if (!owner) {
      throw new Error(
        `Variant modification owner not found for ${sectionAddress(section)} ` +
        `(target &${targetFileId}).`
      );
    }
