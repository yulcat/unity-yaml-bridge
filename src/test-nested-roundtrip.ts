...
import { readCompact, CompactProperty } from './compact-reader';
...
`;
  ...
  const parsed = readCompact(compact);
  ...
  const modelChildren = model.value as CompactProperty[];
  ...
  const combatChildren = modelChildren[0].value as CompactProperty[];
  ...
  const fontChildren = fontData.value as CompactProperty[];
...
`;
  ...
  assert(compactStr.includes('name = Sword') || compactStr.includes('name: Sword'),
    'Sword item serialized correctly');
