...
const ISSUE5_BUTTON_GUID = '4e29b1a8efbd4b44bb3f3716e73f07ff';
...
function issue6NestedButtonSourceYaml(): string {
...
function issue6ParentPrefabYaml(rootFirst: boolean): string {
...
function issue6ParentWithoutRootStubYaml(): string {
  return issue6ParentPrefabYaml(false)
    .replace(/--- !u!1 &1010 stripped\nGameObject:\n(?:  .*\n){3}/, '')
    .replace('  rootGoRef: {fileID: 1010}\n', '  rootGoRef: {fileID: 0}\n');
}

function issue6VariantYaml(): string {
  return issue5VariantParentYaml().replace(
    '    m_RemovedComponents: []',
    `    - target: {fileID: 300, guid: ${ISSUE5_BASE_GUID}, type: 3}
...
function makeIssue6Resolver(parentYaml: string): GuidResolver {
  ...
  const issue6Resolver = new GuidResolver();
...
for (const rootFirst of [false, true]) {
  ...
  const ast = parseUnityYaml(parentYaml);
  const compactStr = writeCompact(ast, { guidResolver: issue6Resolver });
  const compact = readCompact(compactStr);
  ...
  const noProjectCompact = readCompact(writeCompact(ast));
  if (noProjectCompact.refs.get(rootKey)?.[0] === '1010') {
    ...
    fail('No-project nested root identity', writeCompact(ast));
  }
  ...
  if (String(controller?.properties.rootGoRef?.fileID) === '1010' &&
    ...
    fail('Issue #6 no-edit roundtrip', writeUnityYaml(merged));
  }
}

{
  ...
  const issue6Resolver = makeIssue6Resolver(parentYaml);
  const compact = readCompact(writeCompact(parseUnityYaml(parentYaml), { guidResolver: issue6Resolver }));
...

{
  ...
  const issue6Resolver = makeIssue6Resolver(parentYaml);
  const ast = parseUnityYaml(issue6VariantYaml());
  const compactStr = writeCompact(ast, { guidResolver: issue6Resolver });
  const compact = readCompact(compactStr);
  ...
  if (String(rootMod?.objectReference.fileID) === '1010' &&
    ...
    fail('Variant Issue #6 no-edit roundtrip', writeUnityYaml(merged));
  }
}
