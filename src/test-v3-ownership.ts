...

{
  ...
  const removedInternal = parseUnityYaml(writeUnityYaml(compileV3(document)));
  ...
  assert(removedInternalDelta.length === 1 &&
         ...
         removedInternalDelta[0].guid === internalGameObject?.sourceGuid,
    'removing an inherited nested internal emits its nested-source GameObject delta');
  inheritedNested.children = [internalChild];
  internalChild.tombstone = true;
  internalChild.children = [];
  const tombstonedInternal = parseUnityYaml(writeUnityYaml(compileV3(document)));
  ...
  assert(tombstonedInternalDelta.length === 1 &&
         ...
         tombstonedInternalDelta[0].guid === internalGameObject?.sourceGuid,
    'an explicit inherited nested internal tombstone emits its nested-source GameObject delta');
...

{
  ...
  try {
    ...
    const directDistinctOwnerComponent = document.identity.get(
      document.variantRoots![0].components[0].machineId
    )!;
    ...
    const distinctOwnerRebuilt = parseUnityYaml(writeUnityYaml(compileV3(document)));
    assert(!directDistinctOwnerComponent.prefabOwnerId && !!innerDistinctOwnerComponent.prefabOwnerId &&
           distinctOwnerRebuilt.documents.length === variant.documents.length,
      'equal inherited source tuples under distinct direct PrefabInstance owners remain valid');
    ...
    const objectEdited = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const objectDeltas = objectEdited.prefabInstances[0].modifications.filter(modification =>
    ...
    const exportedObject = readV3(writeV3(objectEdited, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedColorComponent = [...exportedObject.identity.values()].find(identity =>
      identity.kind === 'component' && identity.sourceGuid === colorComponent.sourceGuid &&
    ...
    assert(JSON.stringify(exportedColorDetails.m_Color) === JSON.stringify({ a: 0.5, b: 0.25 }) &&
           JSON.stringify(exportedColorDetails.m_RaycastPadding) === JSON.stringify({ w: 4, x: 1 }),
      'existing scalar leaf modifications export as partial Color and Vector objects without baseline leaves');
    const coldObject = parseUnityYaml(writeUnityYaml(compileV3(exportedObject)));
    ...
    const unprovenFlat = parseUnityYaml(writeUnityYaml(objectEdited));
    ...
    const exportedFlat = readV3(writeV3(unprovenFlat, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedFlatIdentity = [...exportedFlat.identity.values()].find(identity =>
      identity.kind === 'component' && identity.sourceGuid === colorComponent.sourceGuid &&
      identity.sourceFileId === colorComponent.sourceFileId
    )!;
    assert(exportedFlat.details.get(exportedFlatIdentity.machineId)?.['m_Unproven.x'] === 7,
      'an unproven scalar leaf modification remains a deterministic flat property path');
    const coldFlat = parseUnityYaml(writeUnityYaml(compileV3(exportedFlat)));
    assert(coldFlat.prefabInstances[0].modifications.some(modification =>
      modification.propertyPath === 'm_Unproven.x' && modification.value === '7' &&
      String(modification.target.fileID) === colorComponent.sourceFileId &&
      modification.target.guid === colorComponent.sourceGuid
    ...
    expectThrow(
      () => writeV3(overlappingExport, {
        sourceResolver: {
          resolveFilePath: guid => guid === outerGuid ? outerPath :
            guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
        },
      }),
      'overlaps another projected property path',
      'export rejects overlapping object-reference and scalar leaf property paths'
    );
    exportedObject.details.set(exportedColorComponent.machineId, {
      m_Color: { a: 0.125, nested: { enabled: true } },
    });
    const replacedObject = parseUnityYaml(writeUnityYaml(compileV3(exportedObject)));
    const replacedObjectDeltas = replacedObject.prefabInstances[0].modifications.filter(modification =>
      String(modification.target.fileID) === colorComponent.sourceFileId &&
      modification.target.guid === colorComponent.sourceGuid
    );
    ...
    const removedObject = parseUnityYaml(writeUnityYaml(compileV3(exportedObject)));
    assert(!removedObject.prefabInstances[0].modifications.some(modification =>
      String(modification.target.fileID) === colorComponent.sourceFileId &&
      modification.target.guid === colorComponent.sourceGuid &&
      modification.propertyPath.startsWith('m_Color.')
    ), 'removing a partial object removes all corresponding leaf modifications');
    ...
    const nullEdited = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const nullDelta = nullEdited.prefabInstances[0].modifications.find(modification =>
    ...
    const exportedEmptyScalarIdentity = [...exportedEmptyScalar.identity.values()].find(identity =>
      identity.kind === 'gameObject' && identity.sourceGuid === innerRoot.sourceGuid &&
      identity.sourceFileId === innerRoot.sourceFileId
    )!;
    assert(exportedEmptyScalar.details.get(exportedEmptyScalarIdentity.machineId)?.m_CustomEmpty === '',
      'empty scalar override remains an empty string when the source baseline is not a reference');

    const fontComponent = [...document.identity.values()].find(identity =>
      identity.kind === 'component' && identity.sourceGuid === innerGuid &&
      identity.sourceFileId === '6714972992410759118'
    )!;
    const dottedNullEdited = parseUnityYaml(writeUnityYaml(variant));
    ...
    const exportedDottedNull = readV3(writeV3(dottedNullEdited, nullExportOptions));
    const exportedFontIdentity = [...exportedDottedNull.identity.values()].find(identity =>
      identity.kind === 'component' && identity.sourceGuid === fontComponent.sourceGuid &&
    ...
    const exportedFontData: any = exportedFontDetails.m_FontData;
    const coldDottedNull = parseUnityYaml(writeUnityYaml(compileV3(exportedDottedNull)));
    const coldDottedNullDeltas = coldDottedNull.prefabInstances[0].modifications.filter(modification =>
      String(modification.target.fileID) === fontComponent.sourceFileId &&
    ...
    assert(exportedFontDetails['m_FontData.m_Font'] === null &&
           ...
           coldDottedNullDeltas.some(modification =>
             modification.propertyPath === 'm_FontData.m_Font' && modification.value === '' &&
             String(modification.objectReference.fileID) === '0') &&
           coldDottedNullDeltas.some(modification =>
             modification.propertyPath === 'm_FontData.m_FontSize' && modification.value === '' &&
             String(modification.objectReference.fileID) === '0'),
      'dotted null references use the resolved baseline leaf while dotted empty scalars remain scalar');
    ...
    for (const propertyPath of ['m_GameObject', 'm_GameObject.fileID']) {
      ...
      expectThrow(
        () => compileV3(document),
        'is structural and not supported',
        `raw inherited nested modification rejects structural ${propertyPath === 'm_GameObject' ? 'roots' : 'descendants'}`
      );
      rawNestedModifications.pop();
    }
    for (const [propertyPath, expected] of [
      ...
      expectThrow(
        () => compileV3(document), expected,
        `raw inherited nested modification rejects ${propertyPath.includes('__proto__') ? 'prototype-sensitive' : 'invalid'} paths`
      );
      rawNestedModifications.pop();
    }
    rawNestedModifications.push(
    ...
    expectThrow(
      ...
      'raw inherited nested modifications reject segment-prefix conflicts'
    );
    ...
    const externalEdited = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const externalDelta = externalEdited.prefabInstances[0].modifications.find(modification =>
      modification.propertyPath === 'm_Icon' &&
      String(modification.target.fileID) === innerRoot.sourceFileId &&
      modification.target.guid === innerRoot.sourceGuid
    );
    assert(externalDelta?.value === '' &&
           ...
           externalDelta.objectReference.type === 3,
      'explicit external DETAILS reference compiles unchanged at arbitrary nested depth');
    const exportedExternal = readV3(writeV3(externalEdited, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedExternalIdentity = [...exportedExternal.identity.values()].find(identity =>
      identity.kind === 'gameObject' && identity.sourceGuid === innerRoot.sourceGuid &&
      identity.sourceFileId === innerRoot.sourceFileId
    )!;
    ...
    const stringFileIdExternal = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const stringFileIdDelta = stringFileIdExternal.prefabInstances[0].modifications.find(modification =>
      modification.propertyPath === 'm_Icon' &&
      String(modification.target.fileID) === innerRoot.sourceFileId &&
      modification.target.guid === innerRoot.sourceGuid
    );
    assert(String(stringFileIdDelta?.objectReference.fileID) === '21300000',
      'explicit external reference accepts a canonical string fileID');

    document.details.set(innerRoot.machineId, { m_Icon: { $ref: deepComponent.machineId } });
    const inheritedReferenceEdited = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const inheritedReferenceDelta = inheritedReferenceEdited.prefabInstances[0].modifications.find(modification =>
      modification.propertyPath === 'm_Icon' &&
      String(modification.target.fileID) === innerRoot.sourceFileId &&
      modification.target.guid === innerRoot.sourceGuid
    );
    assert(inheritedReferenceDelta?.value === '' &&
           ...
           inheritedReferenceDelta.objectReference.type === 3,
      'stable reference to an inherited identity uses its exact source GUID and fileID');
    const exportedInheritedReference = readV3(writeV3(inheritedReferenceEdited, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedInheritedOwner = [...exportedInheritedReference.identity.values()].find(identity =>
      identity.kind === 'gameObject' && identity.sourceGuid === innerRoot.sourceGuid &&
      identity.sourceFileId === innerRoot.sourceFileId
    )!;
    const exportedInheritedTarget = [...exportedInheritedReference.identity.values()].find(identity =>
      identity.kind === 'component' && identity.sourceGuid === deepComponent.sourceGuid &&
    ...
    assert(JSON.stringify(exportedInheritedReference.details.get(exportedInheritedOwner.machineId)?.m_Icon) ===
           JSON.stringify({ $ref: exportedInheritedTarget.machineId }),
      'existing inherited object reference exports as one unambiguous stable machine reference');
    const coldInheritedReference = parseUnityYaml(writeUnityYaml(compileV3(exportedInheritedReference)));
    const coldInheritedDelta = coldInheritedReference.prefabInstances[0].modifications.find(modification =>
      modification.propertyPath === 'm_Icon' &&
    ...
    assert(!coldInheritedDelta,
      'exported inherited stable reference equal to its baseline suppresses a redundant leaf delta');
    ...
    const replacedInheritedReference = parseUnityYaml(
      writeUnityYaml(compileV3(exportedInheritedReference))
    );
    const replacedInheritedDelta = replacedInheritedReference.prefabInstances[0].modifications.find(
      modification => modification.propertyPath === 'm_Icon' &&
        String(modification.target.fileID) === exportedInheritedOwner.sourceFileId &&
        modification.target.guid === exportedInheritedOwner.sourceGuid
    );
    assert(replacedInheritedDelta?.objectReference.guid === externalReference.guid,
      'exported inherited reference can be replaced by an explicit external reference');
    exportedInheritedReference.details.delete(exportedInheritedOwner.machineId);
    const removedInheritedReference = parseUnityYaml(
      writeUnityYaml(compileV3(exportedInheritedReference))
    );
    assert(!removedInheritedReference.prefabInstances[0].modifications.some(modification =>
      modification.propertyPath === 'm_Icon' &&
      String(modification.target.fileID) === exportedInheritedOwner.sourceFileId &&
      modification.target.guid === exportedInheritedOwner.sourceGuid
    ), 'removing exported reference DETAILS removes the leaf modification');

    document.details.set(innerRoot.machineId, { m_Icon: { $ref: document.variantRootId } });
    const localReferenceEdited = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const localReferenceDelta = localReferenceEdited.prefabInstances[0].modifications.find(modification =>
      modification.propertyPath === 'm_Icon' &&
      String(modification.target.fileID) === innerRoot.sourceFileId &&
      modification.target.guid === innerRoot.sourceGuid
    );
    assert(localReferenceDelta?.value === '' &&
           ...
           !localReferenceDelta.objectReference.guid,
      'stable reference to an emitted local identity uses its allocated local fileID');
    const exportedLocalReference = readV3(writeV3(localReferenceEdited, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedLocalOwner = [...exportedLocalReference.identity.values()].find(identity =>
      identity.kind === 'gameObject' && identity.sourceGuid === innerRoot.sourceGuid &&
      identity.sourceFileId === innerRoot.sourceFileId
    )!;
    assert(JSON.stringify(exportedLocalReference.details.get(exportedLocalOwner.machineId)?.m_Icon) ===
           JSON.stringify({ $ref: exportedLocalReference.variantRootId }),
      'existing local object reference exports as its unique stable machine identity');
    ...
    expectThrow(
      () => compileV3(document),
      'Invalid v3 object reference',
      'external reference override rejects extra keys'
    );
    document.details.set(innerRoot.machineId, {
    ...
    expectThrow(
      () => compileV3(document),
      'Invalid v3 machine reference',
      'stable reference override rejects extra keys'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { guid: 'label', type: 3 } });
    const ordinaryGuidTypeObject = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const ordinaryGuidTypeDeltas = ordinaryGuidTypeObject.prefabInstances[0].modifications.filter(modification =>
      String(modification.target.fileID) === innerRoot.sourceFileId &&
      modification.target.guid === innerRoot.sourceGuid &&
    ...
    expectThrow(
      () => compileV3(document),
      'requires a nonempty primitive object',
      'primitive object override rejects arrays'
    );
    document.details.set(innerRoot.machineId, { m_Custom: {} });
    expectThrow(
      () => compileV3(document),
      'requires a nonempty primitive object',
      'primitive object override rejects empty objects'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { nested: null } });
    expectThrow(
      () => compileV3(document),
      'requires a primitive leaf',
      'primitive object override rejects null leaves'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { nested: [1] } });
    expectThrow(
      () => compileV3(document),
      'requires a primitive leaf',
      'primitive object override rejects nested arrays'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { nested: { fileID: 1 } } });
    expectThrow(
      () => compileV3(document),
      'reference shape',
      'primitive object override rejects nested fileID reference shapes'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { nested: { $ref: deepComponent.machineId } } });
    expectThrow(
      () => compileV3(document),
      'reference shape',
      'primitive object override rejects nested stable reference shapes'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { nested: Number.POSITIVE_INFINITY } });
    expectThrow(
      () => compileV3(document),
      'requires a finite number',
      'primitive object override rejects nonfinite number leaves'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { 'bad.key': 1 } });
    expectThrow(
      () => compileV3(document),
      'unsafe object key',
      'primitive object override rejects path-delimiter keys'
    );
    document.details.set(innerRoot.machineId, { m_Custom: { '': 1 } });
    expectThrow(
      () => compileV3(document),
      'unsafe object key',
      'primitive object override rejects empty nested keys'
    );
    document.details.set(innerRoot.machineId, { '': 1 });
    expectThrow(
      () => compileV3(document),
      'empty property path',
      'nested DETAILS rejects an empty root property path'
    );
    class NonPlainOverride { leaf = 1; }
    document.details.set(innerRoot.machineId, { m_Custom: new NonPlainOverride() });
    expectThrow(
      () => compileV3(document),
      'plain JSON object',
      'primitive object override rejects non-plain object instances'
    );
    ...
    expectThrow(
      () => compileV3(document),
      'is structural and not supported',
      'primitive object override rejects a structural property root'
    );
    document.details.set(innerRoot.machineId, { 'm_GameObject.fileID': 1 });
    expectThrow(
      () => compileV3(document),
      'is structural and not supported',
      'primitive object override rejects structural descendant paths'
    );
    document.details.set(innerRoot.machineId, {
      m_Color: { r: 1 },
      'm_Color.r': 0.5,
    ...
    expectThrow(
      ...
      'nested override rejects dangling stable references'
    );
    document.details.set(innerRoot.machineId, { m_Icon: { $ref: deepComponent.machineId } });
    const deepComponentEntry = innerBoundary.components.find(component =>
      component.machineId === deepComponent.machineId
    )!;
    innerBoundary.components = innerBoundary.components.filter(component =>
      component.machineId !== deepComponent.machineId
    );
    expectThrow(
      () => compileV3(document),
      'not an effective identity',
      'nested override rejects references to omitted inherited targets'
    );
    ...
    expectThrow(
      () => compileV3(document),
      'duplicate inherited owner/source identity',
      'global identity index rejects ambiguous inherited stable-reference targets before reconciliation'
    );
    ...
    const deepAddedFirst = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const deepAddedSecond = parseUnityYaml(writeUnityYaml(compileV3(document)));
    ...
    const deepAddedGameObject = deepAddedFirst.documents.find(item =>
      item.properties.m_Name === 'DeepAdded'
    )!;
    const deepAddedTransform = deepAddedFirst.documents.find(item =>
      String(item.properties.m_GameObject?.fileID) === deepAddedGameObject.fileId
    )!;
    const deepParentTransform = [...document.identity.values()].find(identity =>
      identity.kind === 'transform' && identity.ownerId === innerBoundary.machineId
    )!;
    const deepParentStub = deepAddedFirst.documents.find(item =>
      item.stripped && (item.typeId === 4 || item.typeId === 224) &&
    ...
    assert(String(deepAddedDelta.targetCorrespondingSourceObject.fileID) === deepParentTransform.sourceFileId &&
           ...
           !deepAddedFirst.documents.some(item => item.fileId === innerRoot.sourceFileId),
      'adding a GameObject below a recursively expanded inherited parent emits a deterministic leaf-owned delta and stripped parent proxy');

    const exportedDeepGameObject = readV3(writeV3(deepAddedFirst, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedOuterBoundary = exportedDeepGameObject.variantRoots![0].children.find(
      node => node.nestedSourceGuid === middleGuid
    )!;
    const exportedInnerBoundary = exportedOuterBoundary.children.find(
      node => node.nestedSourceGuid === innerGuid
    )!;
    ...
    const exportedDeepAddedTransform = [...exportedDeepGameObject.identity.values()].find(identity =>
      identity.kind === 'transform' && identity.ownerId === exportedDeepAdded.machineId
    )!;
    const recompiledDeepGameObject = parseUnityYaml(writeUnityYaml(compileV3(exportedDeepGameObject)));
    ...
    assert(exportedDeepAddedIdentity.origin !== 'inherited' &&
           ...
           recompiledDeepGameObjectDelta.targetCorrespondingSourceObject.guid ===
             deepParentTransform.sourceGuid,
      'an existing deep m_AddedGameObjects delta projects into the same effective STRUCTURE and cold-roundtrips');
    exportedInnerBoundary.children = exportedInnerBoundary.children.filter(node =>
      node.machineId !== exportedDeepAdded.machineId
    );
    const removedExportedDeepGameObject = parseUnityYaml(writeUnityYaml(
      compileV3(exportedDeepGameObject)
    ));
    assert(removedExportedDeepGameObject.documents.find(item => item.typeId === 1001)!
             .properties.m_Modification.m_AddedGameObjects.length === 0 &&
           !removedExportedDeepGameObject.documents.some(item =>
             item.fileId === exportedDeepAddedIdentity.fileId ||
             item.fileId === exportedDeepAddedTransform.fileId
           ),
      'removing an exported deep added GameObject removes its delta and local documents');
    ...
    const deepComponentFirst = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const deepComponentSecond = parseUnityYaml(writeUnityYaml(compileV3(document)));
    const addedComponentDelta = deepComponentFirst.prefabInstances[0].addedComponents[0];
    const addedComponentDocument = deepComponentFirst.documents.find(item =>
      item.typeId === 65 && !item.stripped
    )!;
    const addedComponentStub = deepComponentFirst.documents.find(item =>
      item.typeId === 1 && item.stripped &&
    ...
    assert(String(addedComponentDelta.targetGameObject.fileID) === innerRoot.sourceFileId &&
           ...
           !deepComponentFirst.documents.some(item => item.fileId === innerRoot.sourceFileId),
      'adding a component to a recursively expanded inherited GameObject emits a deterministic leaf-owned delta and stripped GameObject proxy');

    const exportedDeepComponent = readV3(writeV3(deepComponentFirst, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const exportedComponentOuter = exportedDeepComponent.variantRoots![0].children.find(
      node => node.nestedSourceGuid === middleGuid
    )!;
    const exportedComponentInner = exportedComponentOuter.children.find(
      node => node.nestedSourceGuid === innerGuid
    )!;
    const exportedAddedComponent = exportedComponentInner.components.find(component =>
      exportedDeepComponent.identity.get(component.machineId)?.fileId === addedComponentDocument.fileId
    )!;
    const exportedAddedComponentIdentity = exportedDeepComponent.identity.get(
      exportedAddedComponent.machineId
    )!;
    const recompiledDeepComponent = parseUnityYaml(writeUnityYaml(compileV3(exportedDeepComponent)));
    ...
    assert(exportedAddedComponentIdentity.origin !== 'inherited' &&
           ...
           recompiledDeepComponentDelta.targetGameObject.guid === innerRoot.sourceGuid,
      'an existing deep m_AddedComponents delta projects into the same effective STRUCTURE and cold-roundtrips');
    exportedComponentInner.components = exportedComponentInner.components.filter(component =>
      component.machineId !== exportedAddedComponent.machineId
    );
    const removedExportedDeepComponent = parseUnityYaml(writeUnityYaml(
      compileV3(exportedDeepComponent)
    ));
    assert(removedExportedDeepComponent.prefabInstances[0].addedComponents.length === 0 &&
           !removedExportedDeepComponent.documents.some(item =>
             item.fileId === exportedAddedComponentIdentity.fileId
           ) &&
           !removedExportedDeepComponent.documents.some(item => item.stripped && item.typeId === 1 &&
             ...
             item.properties.m_CorrespondingSourceObject?.guid === innerRoot.sourceGuid),
      'removing an exported deep added component removes its delta, document, and stripped proxy');
    ...
    const innerTransform = [...document.identity.values()].find(identity =>
      identity.kind === 'transform' && identity.ownerId === innerBoundary.machineId
    )!;
    ...
    expectThrow(
      () => compileV3(document),
      'ambiguous owner/source path',
      'deep component addition rejects an ambiguous repeated nested-source GameObject target'
    );
    innerBoundary.components.pop();
    document.details.delete('cDeepAdded');
    ...
    const componentRemoved = parseUnityYaml(writeUnityYaml(compileV3(document)));
    ...
    assert(deepRemovedComponents.length === 1 &&
           ...
           componentRemoved.documents.length === variant.documents.length,
      'removing a recursively expanded inherited nested component emits one leaf-owned source delta and preserves identity');

    const exportedComponentRemoval = readV3(writeV3(componentRemoved, {
      sourceResolver: {
        resolveFilePath: guid => guid === outerGuid ? outerPath :
          guid === middleGuid ? middlePath : guid === innerGuid ? innerPath : undefined,
      },
    }));
    const recompiledComponentRemoval = parseUnityYaml(writeUnityYaml(
      compileV3(exportedComponentRemoval)
    ));
    const recompiledComponentRemovalDeltas =
      recompiledComponentRemoval.prefabInstances[0].removedComponents;
    const exportedRemovedComponentIdentity = [...exportedComponentRemoval.identity.values()].find(identity =>
    ...
    const exportedEffectiveComponentIds = new Set<string>();
    const collectEffectiveComponentIds = (node: typeof innerBoundary): void => {
    ...
    assert(recompiledComponentRemovalDeltas.length === 1 &&
           ...
           !recompiledComponentRemoval.documents.some(item =>
             item.fileId === removedDeepComponentIdentity.sourceFileId
           ),
      'an existing deep nested component removal exports and cold-roundtrips without source documents');
    ...
    expectThrow(
      () => compileV3(document),
      'ambiguous owner/source path',
      'single-sided duplicate nested component removal target fails closed'
    );
    document.identity.delete('singleSidedDuplicateNestedComponent');

    const outerComponentIndex = outerBoundary.components.findIndex(component =>
      component.machineId !== removedDeepComponent.machineId
    );
    ...
    expectThrow(
      () => compileV3(document),
      'ambiguous owner/source path',
      'duplicate recursively expanded inherited nested component removal targets fail closed'
    );
    ...
    expectThrow(
      () => compileV3(document),
      'is not a prefabInstance identity',
      'recursively expanded inherited nested removal rejects a missing PrefabInstance owner chain'
    );
    outerPrefab.prefabOwnerId = outerPrefab.machineId;
    expectThrow(
      () => compileV3(document),
      'cyclic PrefabInstance owner path',
      'recursively expanded inherited nested removal rejects a cyclic PrefabInstance owner chain'
    );
    ...
    const removedDeepDescendantIds = new Set<string>();
    const collectRemovedIdentity = (node: typeof removedDeepChild): void => {
      ...
      if (transform) removedDeepDescendantIds.add(transform.machineId);
      node.children.forEach(collectRemovedIdentity);
    };
    collectRemovedIdentity(removedDeepChild);

    const removedDeepTransformIdentity = [...document.identity.values()].find(identity =>
      identity.kind === 'transform' && identity.ownerId === removedDeepChild.machineId
    )!;
    ...
    expectThrow(
      () => compileV3(document),
      'ambiguous owner/source path',
      'duplicate recursively expanded inherited nested GameObject removal targets fail closed'
    );
    document.identity.delete('ambiguousRemovedNestedGo');
    document.identity.delete('ambiguousRemovedNestedTransform');

    const deeplyRemoved = parseUnityYaml(writeUnityYaml(compileV3(document)));
    ...
    const recompiledDeepRemoval = parseUnityYaml(writeUnityYaml(compileV3(exportedDeepRemoval)));
    const recompiledDeepRemovalDeltas = recompiledDeepRemoval.prefabInstances[0].removedGameObjects;
    const exportedRemovedIdentity = [...exportedDeepRemoval.identity.values()].find(identity =>
      identity.kind === 'gameObject' && identity.sourceGuid === removedDeepIdentity.sourceGuid &&
      identity.sourceFileId === removedDeepIdentity.sourceFileId
    );
    const exportedNodes = new Map<string, typeof removedDeepChild>();
    const collectExportedNodes = (node: typeof removedDeepChild): void => {
    ...
    assert(recompiledDeepRemovalDeltas.length === 1 &&
           ...
           !recompiledDeepRemoval.documents.some(item =>
             item.fileId === removedDeepIdentity.sourceFileId ||
             item.fileId === removedDeepTransformIdentity.sourceFileId
           ),
      'an existing deep nested GameObject removal exports and cold-roundtrips without source documents');
    ...
    expectThrow(
      () => compileV3(document),
      'ambiguous owner/source path',
      'single-sided duplicate nested GameObject removal target fails closed'
    );
    innerBoundary.children.pop();
    ...
    const sourceRootRemoved = parseUnityYaml(writeUnityYaml(compileV3(document)));
    ...
    assert(sourceRootRemovalDeltas.length === 1 &&
           ...
           sourceRootRemoved.documents.length === variant.documents.length,
      'removing a recursively expanded inherited nested source root emits one leaf-owned source delta');
  } finally {
    fs.rmSync(directory, { recursive: true, force: true });
  }
}
...

{
  ...
  for (const invalid of invalidCases) {
    ...
    expectThrow(
      () => compileV3(document),
      invalid.expected,
      `direct inherited DETAILS rejects ${invalid.label} before baseline suppression`
    );
  }

  const overlapping = readV3(sourceBackedVariantText());
  ...
  expectThrow(
    () => compileV3(overlapping),
    'ambiguous owner/source path',
    'direct inherited DETAILS rejects overlapping baseline-equal paths before suppression'
  );

  const structural = readV3(sourceBackedVariantText());
  ...
  expectThrow(
    () => compileV3(structural),
    'is structural and not supported',
    'direct inherited DETAILS rejects a baseline-equal structural path before suppression'
  );
}

{
  ...
  expectThrow(
    ...
    'baseline-equal direct DETAILS rejects an exact preserved raw modification conflict'
  );
}

{
  ...
  expectThrow(
    () => compileV3(document),
    'overlaps newly authored semantic modification',
    'baseline-equal direct DETAILS rejects a preserved raw parent conflict'
  );
}

{
  ...
  expectThrow(
    () => compileV3(document),
    'overlaps newly authored semantic modification',
    'baseline-equal direct DETAILS rejects a preserved raw child conflict'
  );
}

{
  ...
  const compiled = parseUnityYaml(writeUnityYaml(compileV3(document)));
  const targetDeltas = compiled.prefabInstances[0].modifications.filter(modification =>
    String(modification.target.fileID) === identity.sourceFileId &&
    modification.target.guid === identity.sourceGuid
  );
  assert(!targetDeltas.some(modification => modification.propertyPath === 'm_Equal') &&
         targetDeltas.some(modification =>
           modification.propertyPath === 'm_Changed' && modification.value === '2') &&
         targetDeltas.some(modification =>
           modification.propertyPath === 'm_RawOnly' && modification.value === 'preserved'),
    'nonconflicting baseline-equal semantic values suppress while changed values emit beside preserved raw modifications');
}

{
  ...
  const suppressed = parseUnityYaml(writeUnityYaml(compileV3(document)));
  const suppressedDeltas = suppressed.prefabInstances[0].modifications.filter(modification =>
    String(modification.target.fileID) === identity.sourceFileId &&
    modification.target.guid === identity.sourceGuid
  );
  assert(suppressedDeltas.length === 0,
    'validated baseline-equal scalar, boolean, null, references, and partial object leaves suppress direct deltas');
  ...
  const changed = parseUnityYaml(writeUnityYaml(compileV3(document)));
  const changedDeltas = changed.prefabInstances[0].modifications.filter(modification =>
    String(modification.target.fileID) === identity.sourceFileId &&
    modification.target.guid === identity.sourceGuid
  );
  ...
  ].every(propertyPath => changedPaths.has(propertyPath)) && changedDeltas.length === 8,
  'type-changed and value-changed direct DETAILS emit exact scalar, reference, and partial-object deltas');
}
...

{
  ...
  assert(modification.m_AddedGameObjects.length === 1 &&
         modification.m_AddedComponents.length === 0 && !!addedComponent,
    'one edit can add a local child with a local component below inherited hierarchy');
}
...

{
  ...
  expectThrow(
    () => compileV3(document),
    'unsafe property path segment',
    'unknown-GUID raw modifications reject unsafe property paths before identity routing'
  );
}

{
  ...
  expectThrow(
    () => compileV3(document),
    'mixes a nonempty scalar value with a nonzero objectReference',
    'unknown-GUID raw modifications reject mixed scalar/reference envelopes before identity routing'
  );
}

{
  ...
  expectThrow(
    () => compileV3(document),
    'structural and not supported',
    'unknown-GUID raw modifications reject structural property paths before identity routing'
  );
}

{
  ...
  const rebuilt = parseUnityYaml(writeUnityYaml(compileV3(document)));
  const preserved = rebuilt.prefabInstances[0].modifications.find(modification =>
    modification.target.guid === unrelated.target.guid &&
    String(modification.target.fileID) === unrelated.target.fileID &&
    modification.propertyPath === unrelated.propertyPath
  );
  assert(preserved?.value === unrelated.value && String(preserved.objectReference.fileID) === '0',
    'valid unrelated-GUID raw modifications remain preserved after generic envelope validation');
}

{
  const malformedUnknownEntries: Array<[string, Record<string, unknown>, string]> = [
    ['non-3 target type', {
  ...
  for (const [label, entry, expected] of malformedUnknownEntries) {
    ...
    expectThrow(
      () => compileV3(document),
      expected,
      `unknown-GUID raw modification rejects ${label} before identity routing`
    );
  }
}

{
  ...
  details.m_Modification.m_Modifications.push(
    { target: { ...target }, propertyPath: 'm_Overlap', value: '1', objectReference: { fileID: 0 } },
    { target: { ...target }, propertyPath: 'm_Overlap.child', value: '2', objectReference: { fileID: 0 } },
  );
  expectThrow(
    () => compileV3(document),
    'overlaps another property path',
    'unknown-GUID raw modifications reject segment-prefix overlaps before identity routing'
  );
}
  ...
  for (const field of listFields) {
    for (const [label, malformed] of malformedContainers) {
      ...
      expectThrow(
        ...
        `variant compiler rejects ${label} ${field} before desired-state reconciliation`
      );
    }
  }
}

{
  for (const [label, malformed] of [
    ...
    expectThrow(
      () => compileV3(document),
      'm_Modification must be a plain object',
      `variant compiler rejects ${label} m_Modification before desired-state reconciliation`
    );
  }
}

{
  ...
  for (const [field, expected] of invalidEntries) {
    ...
    expectThrow(
      () => compileV3(document),
      expected,
      `variant compiler rejects malformed ${field} entries before desired-state reconciliation`
    );
  }
}

{
  ...
  assert(rebuilt.documents.length > 0 &&
         ...
         Array.isArray(rootModification.m_AddedComponents),
    'absent variant delta lists remain a valid empty desired-state input');
}

{
  ...
  expectThrow(
    () => compileV3(document),
    'duplicate inherited owner/source identity',
    'hidden duplicate inherited component identity fails before modification and removal reconciliation'
  );
}
