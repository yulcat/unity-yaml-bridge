...
function prefabWithComponentListOnlyMonoBehaviourYaml(): string {
  return `%YAML 1.1
%TAG !u! tag:unity3d.com,2011:
--- !u!1 &100
GameObject:
...
function variantOfComponentListOnlyMonoBehaviourYaml(): string {
  return `%YAML 1.1
%TAG !u! tag:unity3d.com,2011:
--- !u!1001 &900
PrefabInstance:
...
function nestedTextPrefabYaml(): string {
  return `%YAML 1.1
%TAG !u! tag:unity3d.com,2011:
--- !u!1 &100
GameObject:
...
function variantWithNestedPrefabComponentOverridesYaml(): string {
  return `%YAML 1.1
%TAG !u! tag:unity3d.com,2011:
--- !u!1001 &900
PrefabInstance:
...

{
  ...
  const ast = parseUnityYaml(prefabWithComponentListOnlyMonoBehaviourYaml());
  const compact = writeCompact(ast, { guidResolver: resolver });
  const structure = getSection(compact, 'STRUCTURE');

  assert(structure.includes('PopupSize [CanvasGroup, SimpleFSMController]'),
    'STRUCTURE includes SimpleFSMController from the GameObject component list',
    structure);
}

{
  ...
  const ast = parseUnityYaml(variantOfComponentListOnlyMonoBehaviourYaml());
  const compact = writeCompact(ast, { guidResolver: resolver });
  const structure = getSection(compact, 'STRUCTURE');

  assert(structure.includes('PopupSize [CanvasGroup*, SimpleFSMController]'),
    'variant STRUCTURE includes SimpleFSMController from the base prefab component list',
    structure);
}
...

{
  ...
  const ast = parseUnityYaml(variantWithNestedPrefabComponentOverridesYaml());
  ...
  assert(details.includes('[NestedWidget:TextMeshProUGUI]'),
    'DETAILS includes readable nested internal component header',
    details);
  assert(details.includes('m_text = Original nested text'),
    'DETAILS includes nested internal component property override',
    details);
  assert(details.includes('linkRef = ->TargetLabel:TextMeshProUGUI'),
    'DETAILS emits nested internal objectReference as a path ref',
    details);
  assert(refs.includes('NestedWidget:TextMeshProUGUI = 500'),
    'REFS includes nested internal component target',
    refs);
  assert(refs.includes('NestedWidget:TextMeshProUGUI:__instance = 901'),
    'REFS records the owning nested PrefabInstance for edited internals',
    refs);
  assert(refs.includes('TargetLabel:TextMeshProUGUI = 800'),
    'REFS includes nested objectReference target path',
    refs);
  ...
  const merged = mergeCompactChanges(ast, readCompact(editedText));
  ...
  const rootTextMod = rootMods.find((m: any) => m.propertyPath === 'm_text');
  const nestedTextMod = nestedMods.find((m: any) => m.propertyPath === 'm_text');
  const nestedRefMod = nestedMods.find((m: any) => m.propertyPath === 'linkRef');

  assert(nestedTextMod?.value === 'Edited nested text',
    'edited nested internal property writes to non-root PrefabInstance modifications',
    writeUnityYaml(merged));
  assert(rootTextMod?.value === 'Root instance must not change',
    'same fileID/property on root PrefabInstance is not edited by nested section',
    writeUnityYaml(merged));
  assert(fileIdOf(nestedRefMod?.objectReference) === '850',
    'edited nested internal objectReference path writes to non-root PrefabInstance modifications',
    JSON.stringify(nestedRefMod));
}
