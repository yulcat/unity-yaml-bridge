...
function validateUnityModificationObjectReference(
  value: unknown,
  context: string
): { fileId: string; projected: Record<string, unknown> } {
  ...
  const projected = validateV3ExternalObjectReference(objectReference, context);
...
function writeVariantV3(file: UnityFile, options: V3WriterOptions): string {
  ...
  const addedComponentsBySourceGameObject = collectVariantAddedComponents(
    rootInstance, byId
  );
  ...
  const effectiveRoots = inheritedRoots.length > 0 && variantRoots.length > 0
    ? attachVariantAddedRoots(
        variantRoots, inheritedRoots, rootInstance, byId, identities, rootId
      )
  ...
  const directSemanticTargets = new Set([...identities.values()]
    .filter(identity => identity.origin === 'inherited' && !identity.prefabOwnerId &&
      identity.sourceGuid === rootInstance.sourcePrefab.guid && identity.sourceFileId &&
  ...
  for (const document of file.documents) {
    ...
    for (const [key, value] of Object.entries(document.properties)) {
      ...
      const projectedValue = machineId === rootId && key === 'm_Modification' &&
          value && typeof value === 'object'
        ? {
            ...(value as Record<string, unknown>),
            m_Modifications: Array.isArray((value as any).m_Modifications)
              ? (value as any).m_Modifications.filter((entry: any) => !directSemanticTargets.has(
                  `${String(entry?.target?.guid ?? '')}:${String(entry?.target?.fileID ?? '0')}`
                ))
              : (value as any).m_Modifications,
          }
        : value;
      lines.push(`${key} = ${formatV3Value(encodeV3References(projectedValue, documentIds))}`);
    }
  }
...
function attachVariantAddedRoots(
  localRoots: V3StructureNode[],
  inheritedRoots: V3StructureNode[],
  ...
  const inheritedNodesByTransform = new Map<string, V3StructureNode>();
  ...
  const rootTransformIdentity = (node: V3StructureNode): V3IdentityRecord => {
    const matches = [...identities.values()].filter(identity =>
  ...
  for (const localRoot of localRoots) {
    ...
    if (!addition) {
      throw new Error(
        `Variant-added root ${localRoot.machineId} is missing direct m_AddedGameObjects ownership.`
      );
    }
  ...
  if (additionsByObject.size > 0) {
    throw new Error(
      `Variant m_AddedGameObjects addedObject ${additionsByObject.keys().next().value} ` +
...
function buildInheritedVariantRoots(
  ...
  const matchRemoval = (
    ...
    const key = `${guid}:${fileId}`;
    if (!removals.has(key)) return false;
    if (matched.has(key)) {
      throw new Error(
        `Variant root ${rootInstance.fileId} has ambiguous ${label} ownership for ${key}.`
      );
    }
    matched.add(key);
    return true;
  ...
  for (const modification of rootInstance.modifications) {
    ...
    const fileId = String(modification.target.fileID ?? '0');
    const directDocument = guid === sourceGuid ? sourceById.get(fileId) : undefined;
    if (directDocument && (directDocument.typeId === 4 || directDocument.typeId === 224)) continue;
  ...
  const buildNestedInternal = (
    ...
    if (node.nestedPrefab) {
      ...
      const childSource = resolveEffectiveVariantSource(
        childSourceGuid, options, new Set(resolvingSources)
      );
      const root = buildNestedInternal(
        childSource.hierarchy,
        childSourceGuid,
        nestedPrefabInstanceId,
        childSource.documents,
    ...
    const tombstone = matchRemoval(
      removedGameObjectIds, matchedRemovedGameObjectIds,
      nestedSourceGuid, node.fileId, 'removed-GameObject'
    );
    identities.set(goId, {
      machineId: goId,
      kind: 'gameObject',
    ...
    const children = node.children.map((child, index) => buildNestedInternal(
      child, nestedSourceGuid, prefabOwnerId, nestedDocuments, transformId, index, resolvingSources
    ));
    return {
      name: effectiveName,
      machineId: goId,
  ...
  const build = (
    ...
    const tombstone = matchRemoval(
      removedGameObjectIds, matchedRemovedGameObjectIds,
      sourceGuid, node.fileId, 'removed-GameObject'
    );
    identities.set(goId, {
      machineId: goId,
      kind: 'gameObject',
    ...
    const effectiveName = projectNestedOverrides(
      goId, sourceGuid, node.fileId, node.name, gameObjectDocument.properties
    )!;
    identities.set(transformId, {
      machineId: transformId,
      kind: 'transform',
    ...
    const components = node.components.flatMap((component, index) => {
      ...
      projectNestedOverrides(
        componentId, sourceGuid, component.fileId, undefined, sourceDocument.properties
      );
      if (matchRemoval(
        removedComponentIds, matchedRemovedComponentIds,
        sourceGuid, component.fileId, 'removed-component'
      )) {
        return [];
      }
      return { typeName: component.typeName, machineId: componentId };
  ...
  if (addedComponentsBySourceGameObject.size > 0) {
    throw new Error(
      `Added component target ${addedComponentsBySourceGameObject.keys().next().value} ` +
...
function collectVariantAddedComponents(
  rootInstance: UnityFile['prefabInstances'][number],
  byId: Map<string, UnityDocument>
): Map<string, UnityDocument[]> {
  const result = new Map<string, UnityDocument[]>();
  const seenAddedObjects = new Set<string>();
...
): EffectiveVariantSource {
  if (resolving.has(sourceGuid)) {
    throw new Error(`Variant source chain contains a cycle at ${sourceGuid}.`);
  }
  const sourcePath = options.sourceResolver?.resolveFilePath(sourceGuid);
  if (!sourcePath) throw new Error(`Variant source chain cannot resolve GUID ${sourceGuid}.`);
...
function applyVariantChainRemovedGameObjects(
  hierarchy: GameObjectNode,
  instance: UnityFile['prefabInstances'][number],
  variantGuid: string,
  parentGuid: string
  ...
  for (const reference of instance.removedGameObjects) {
    ...
    if (String(reference.guid ?? '') !== parentGuid || fileId === '0' || removed.has(fileId)) {
      throw new Error(
        `Variant source chain ${variantGuid} has ambiguous removed-GameObject ownership for ${fileId}.`
      );
    }
    const matches = owners.get(fileId) ?? [];
    if (matches.length !== 1) {
      throw new Error(
        `Variant source chain ${variantGuid} removed GameObject ${fileId} is not uniquely owned by its direct source.`
      );
    }
...
function applyVariantChainRemovedComponents(
  hierarchy: GameObjectNode,
  instance: UnityFile['prefabInstances'][number],
  variantGuid: string,
  parentGuid: string
  ...
  for (const reference of instance.removedComponents) {
    ...
    if (String(reference.guid ?? '') !== parentGuid || fileId === '0' || removed.has(fileId)) {
      throw new Error(
        `Variant source chain ${variantGuid} has ambiguous removed-component ownership for ${fileId}.`
      );
    }
    const matches = owners.get(fileId) ?? [];
    if (matches.length !== 1) {
      throw new Error(
        `Variant source chain ${variantGuid} removed component ${fileId} is not uniquely owned by its direct source.`
      );
    }
...
function cloneHierarchy(node: GameObjectNode): GameObjectNode {
  return {
    ...node,
